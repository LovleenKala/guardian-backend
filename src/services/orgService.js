...
/**
 * Robust id extractor:
 *  - accepts ObjectId, 24-hex string, mongoose doc, plain {_id}, or a
 *    stringified object that contains ObjectId("...") **or new ObjectId("...")**
 */
function toId(x) {
  if (!x) return undefined;

  // Already ObjectId / 24-hex
  if (mongoose.isValidObjectId(x)) return String(x);

  // Mongoose doc or object with _id / id
  if (typeof x === 'object') {
    const v = x._id ?? x.id ?? x.orgId ?? x.userId;
    if (mongoose.isValidObjectId(v)) return String(v);
  }

  // String containing various representations
  if (typeof x === 'string') {
    // NOTE: now supports both ObjectId("...") and new ObjectId("...")
    const m = x.match(
...
/**
 * Caretakers are NOT staff. If freelance (no org), link them to this org.
 */
